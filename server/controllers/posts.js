...
postRouter.get("/api/post/:id", (request, response) => {
  Post.find({ _id: request.params.id }).then((posts) => {
    response.json(posts);
  });
});
...
postRouter.put("/api/invest/:id", async (request, response) => {
  ...
  if (post.quantity_remaining < 0) {
    return response.status(401).json({ error: "Cannot invest that amount" });
  }
